...
try:
    for i in range(1,5000):
        ...
        #data = json.dumps({'report_type': "detailed_person_report", 'meta': {'person_id': bvid}})
        #data = {'report_type': "detailed_person_report", 'meta': {'person_id': bvid}}
        ...
        #     'https://www.peoplelooker.com/api/v4/reports', data)#, data=json.dumps({'report_type': "detailed_person_report", 'meta': {'person_id': bvid}}))
        # reports_response = session.get(
        #     'https://www.peoplelooker.com/api/v4/reports', headers={'content-type': 'application/json; charset=utf-8'})#, data=json.dumps({'report_type': "detailed_person_report", 'meta': {'person_id': bvid}}))
        print i, 2, response.status_code,"bvid= "+bvid
        personId=reports_response.json()['reports'][0]['permalink']
        #response = session.get('https://www.peoplelooker.com/api/v4/reports/' + reports_response.json().get('report',{}).get('permalink', ''))
